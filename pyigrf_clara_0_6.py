...
pyIGRF: code to synthesise magnetic field values from the 13th generation of the
 ...
 Dependencies: 
 -------------
     : numpy, scipy, matplotlib, geopandas
...
class IGRF():
    ...
    def _calc_igrf(self,parLat,parLon,parH,parAno):
        ...
        #print("_calc_igrf parLat", parLat," parLon", parLon, "parAno",parAno)
        #print('parlat', parLat,'parH',parH)
        ...
        #print("_calc_igrf lat",lat)
        lat = 90-lat
        
        return  np.round(alt, decimals=3), lat, lon, date, dec, inc, hoz, eff, X, Y, Z, decs, hozs,\
            incs, effs, dX, dY, dZ
    ...
    def calc_grid(self,intervalo_h = 10,lim_h = 500,intervalo_lat=-5,lim_lat=-60,intervalo_lon=-5,lim_lon=-110):
        ...
        print(np.arange(self.entrada_usuario[0],lim_lat,intervalo_lat))
        
        for lat1 in np.arange(self.entrada_usuario[0],lim_lat,intervalo_lat):
            #print("\n\n lat1",lat1)
            for lon in np.arange(self.entrada_usuario[1],lim_lon,intervalo_lon):
                ...
                for h in np.arange(self.entrada_usuario[2],lim_h,intervalo_h):
                    ...
                    d = self._calc_igrf(lat1,lon,h,self.entrada_usuario[3])
                    lgrid.append(d) #quando ele calcula ele já guarda no arquivo o valor daquele ponto
                    #print("\npyigrf_clara_0_2  calc_igrf h,lat,lon :",h,lat1,lon)
    ...
    def going_to_multiindex(self,df):
        df.sort_values(["Altitude",'Latitude','Longitude'],inplace = True)
        df["Latitude"] = np.round(df["Latitude"],5)
        df["Longitude"] = np.round(df["Longitude"],5)
        df["Altitude"] = np.round(df["Altitude"],5)
        
        return df.set_index(["Altitude",'Latitude','Longitude'])
    ...
    def plot_grid(self,Dfgrid,h = 400):
        ...
        #embelezando o mapa
        #ax.set_title("Total Field Intensity in the year " + str(self.entrada_usuario[3]))
        ...
        #plota mapa de cores
        tpc = ax.tripcolor(a["Longitude"],a["Latitude"],a["Total_intensity"],cmap='jet') 
        clb = fig.colorbar(tpc,location='bottom',label='Total Field Intensity (nT)', shrink=0.9)

        countries["geometry"].boundary.plot(ax = ax, color = "gray",figsize=(20,16))
        countries.head()
        
        #plota o mapa de contornos
        #nota: mudar cor das linhas
        ax.tricontour(a["Longitude"],a["Latitude"],a["Total_intensity"],15,colors='black')
...
#
# #========================================
# name_saida = "teste2024" 
# # name_entrada_grid = 'LatinMag2023_grid.csv'
# # #hmax = 500 #a altura maxima para o qual o igrf vai ser calculado

# dado = IGRF(-80,-180,100,2008,name_saida)
# dado.calc_grid(intervalo_h = 20,lim_h = 500,intervalo_lat=10,lim_lat=0,intervalo_lon=10,lim_lon=0)
# dado.Dfgrid = dado.going_to_multiindex(dado.Dfgrid)
#a = dado.get_grid("LatinMag2023_grid.csv")
#print(a)
# dado.get_grid(name_entrada_grid)
# #dado.calc_grid(intervalo_h = 10,lim_h = 500,intervalo_lat=-2,lim_lat=-90,intervalo_lon=-2,lim_lon=-180)
# #print("dado criado")

# #dado.calc_grid(lim_h = 500)
# dado.plot_grid(dado.Dfgrid,490)
