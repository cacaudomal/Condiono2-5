...
class condiono_adachi():
    ...
    def calc_prelativa_all(self, rho_íonO, rho_íonNO, rho_íonO2, ne):
        ...
        self.p1 = self._calc_pRelativa(rho_íonO, ne)
        self.p2 = self._calc_pRelativa(rho_íonNO+rho_íonO2, ne) #relative parameter from ion O+
        print("Done")
    ...
    def calc_Pedersen(self,fen,fin1,fin2,wi1,wi2,we,p1,p2,ne,B):
        ...
        p2 : TYPE
            DESCRIPTION.
        ne : TYPE
            Electrons density [m^-3] .
        B : TYPE
            intensity of earths magnetic field [T].
        rho_íonNO : PANDAS SERIES
            DESCRIPTION.

        Returns
        -------
        self.CondP : PANDA SERIES
            Pedersen Conducctivity [S/m].
    ...
    def plot_2dgrid(self,values,h,title=" ", savemap = True) -> None:
        '''
        Plots data in a 2d grid.

        Parameters
        ----------
        values : PANDAS SERIES
            pandas series with 2d values to be ploted.
        h : FLOAT
            Height [km].
        title : STRING, optional
            Title of the map. The default is " ".
        savemap : BOOLEAN, optional
            WHETHER THE MAP IS TO BE SAVED OR NOT. The default is True.
    ...
    def calc_height_integrtated_conductivity(self,values,h=400):
        ...
        Returns
        -------
        heightintegratedcond : DATA FRAME
            height integrated values matrix in the form lat x lon.
